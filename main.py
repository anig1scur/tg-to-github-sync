...
class TelegramProcessor:
    ...
    async def get_messages_in_range(self, channel: Channel, start_date: datetime, end_date: datetime, limit: int = None) -> List[Message]:
        ...
        async for message in self.client.iter_messages(channel, limit=limit, offset_date=start_date, reverse=True):
    ...
    async def process_messages(self) -> Dict[str, Dict[str, Any]]:
        async with self.client:
            ...
            start_date = (now - timedelta(days=self.config.DAY_LIMIT)).replace(hour=0, minute=0, second=0, microsecond=0)
            end_date = now
    ...
    async def process_message_groups(self, messages: List[Message]) -> Dict[str, Dict[str, Any]]:
        ...
        for message in reversed(messages):
            if not message_group or (message.grouped_id and message.grouped_id == message_group[-1].grouped_id):
...
class GithubUpdater:
    ...
    def update_daily_files(self, month: str, data: Dict[str, Any], element_list: List[InputGitTreeElement]):
    ...
    def update_monthly_file(self, month: str, data: Dict[str, Any], element_list: List[InputGitTreeElement]):
    ...
    def update_media_files(self, month: str, data: Dict[str, Any], element_list: List[InputGitTreeElement]):
        for media_path, local_path in data["media"]:
            with open(local_path, "rb") as file:
                ...
                if self.content_changed(github_media_path, data.decode("utf-8")):
                    element_list.append(InputGitTreeElement(github_media_path, "100644", "blob", sha=blob.sha))
    ...
    def content_changed(self, file_path: str, new_content: str) -> bool:
        try:
            old_content = self.repo.get_contents(file_path, ref=self.config.GITHUB_BRANCH)
    ...
    def create_commit(self, element_list: List[InputGitTreeElement], branch_sha: str, branch_ref: Any):
...
async def main():
    ...
    try:
        ...
        logging.info(f"Successfully updated repository with changes for months: {', '.join(updates.keys())}")
    except Exception as e:
